...
export default function StatCard({ title, value, theme }: StatCardProps) {
  return (
    <div className="flex min-h-[80px] md:h-[100px] w-full items-center justify-between rounded-2xl bg-white p-4 md:p-6 shadow-sm ring-1 ring-gray-100">
      <div className="flex flex-col gap-1">
        <span className="text-[13px] font-medium text-gray-500">{title}</span>
        <span className={cn("text-2xl md:text-3xl font-bold", themeStyles[theme].text)}>
          {value}
        </span>
      </div>
      <div
        className={cn(
          "flex size-10 md:size-14 shrink-0 items-center justify-center rounded-2xl border",
